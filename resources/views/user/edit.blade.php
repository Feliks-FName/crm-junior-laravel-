<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="post" action="{{ route('users.update', $user->id) }}" class="mt-6 space-y-6 max-w-2xl"
                  autocomplete="off">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="role"
                        ...
                        @foreach($roles as $role => $value)
                            <option
                                value="{{ $role }}" {{ $user->role === $role ? 'selected': ''}}>{{ $value }}</option>
                        @endforeach
                    </select>
